...
import { ClassificationResult, ControlSummary } from './ai-provider.interface';
import { embedWithPipeline, getLocalEmbeddingPipeline } from './local-embedding';

// GeminiAiProvider now handles only classifyEvidence (and embed, a
// local/bundled model call with no network dependency at all) — gap-
// analysis coverage checks moved to GroqAiProvider, since Groq's much
// higher free-tier ceiling and speed suit an 18-calls-per-run workload
// far better, and classification specifically needs Gemini's vision
// support for image evidence, which Groq's text-only role doesn't need.
// See CompositeAiProvider for how the two are wired together behind the
// one AiProvider interface callers depend on.
//
...
// that error named, and is what's actually running here. A short
// structured JSON out of a text/vision classification prompt is well
// within its capability, so this pin exists only because 2.5-flash was
// unavailable to this specific account, not because 3.6 was needed on
// its own merits — worth confirming still holds if this key or account
// tier ever changes.
...
// gap-analysis testing (back when coverage checks still ran through
// this same pacer) returned
...
// why this project ended up on 3.6 instead). Now that only
// classifyEvidence uses this pacer (coverage checks moved to Groq's own,
// separate one in GroqAiProvider), 4 (a 20% margin under the confirmed
// 5/min ceiling) is kept as-is — classification's own live-observed
// failure modes (a 503, a 504, a 429) were never traced to this pacer
// being too loose, so there's no evidence it needs revisiting on its
// own. It's a single process-wide pacer (not per-tenant): the
// GEMINI_API_KEY is one project-wide credential shared by every tenant,
// so the thing being protected is the project's quota, not any one
// tenant's.
const GEMINI_FREE_TIER_RPM = 4;
const MIN_CALL_INTERVAL_MS = Math.ceil(60_000 / GEMINI_FREE_TIER_RPM);

// Per-call HTTP timeout/retry budget — see the equivalent constant
...
// transaction's timeout). classifyEvidence runs inline inside the
// evidence-upload transaction (EvidenceTransactionMiddleware — see that
// file for why it needed its own, longer-than-default timeout once this
// was actually measured against a real call instead of FakeAiProvider's
// instant response). It does not block on the pacer (see
// MAX_CLASSIFICATION_PACER_WAIT_MS below) — if the shared pacer is more
// than a few seconds out, it skips the call outright rather than let the
// wait eat further into the transaction budget. No retry beyond the one
// below: EvidenceService.upload already treats any failure here as
// best-effort, plus a retry would itself need to wait for another pacer
// slot.
//
// Text and image classification get DIFFERENT timeouts, not one shared
// value, because their real measured latency is genuinely different in
// kind, not just variance around the same number:
...
//     with the old Claude implementation's numbers, then 12s after the
//     first measurement) each undershot the next real call and aborted
//     it mid-flight.
//   - image: a real image-evidence upload through the actual web UI
//     came back with classification: null — traced end-to-end
//     (temporary logging at every boundary: rate limit consumed, Gemini
//     call reached with the correct inlineData payload, call aborted by
//     OUR OWN 20s client timeout, not a real API error) to this same 20s
//     value being applied to image calls too, which had never been
//     measured on their own — vision input is real, additional work for
//     the model, not noise around the text number. Removing the timeout
//     entirely and re-uploading the same real image measured a genuine
//     ~48s round trip (generateContent called at :20, resolved at :08
//     the next minute) before the model returned a correct, well-formed
//     classification. 90s keeps real margin above that single
//     measurement — the same ~1.5-2x margin the text budget carries
//     over ITS slowest observed call — rather than sitting at the edge
//     of the one data point gathered so far.
...
export class GeminiAiProvider {
  ...
  async classifyEvidence(input: {
    ...
    try {
      ...
      // here. Observed for real during gap-analysis testing (coverage
      // checks hit the same failure mode before they moved to Groq — see
      // GroqAiProvider.checkControlCoverage's equivalent guard): the raw
      // JSON.parse SyntaxError, left unguarded, propagated all the way
      // out of this method as an unrelated-looking parse error instead
      // of the classification failure it actually is.
      const finishReason = response.candidates?.[0]?.finishReason;
      throw new Error(
